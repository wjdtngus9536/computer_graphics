...
GLubyte texImage[][4] = { // width = 5, height = 4, pixel = RGBA (4 byte)
...
void initTex(void) {
	...
	if ((use_new_tex_syntax == true) && glewIsSupported("GL_VERSION_4_5")) {
		...
		glTextureStorage2D(texObj, 1, GL_RGBA8, 5, 4);
		glTextureSubImage2D(texObj, 0, 0, 0, 5, 4, GL_RGBA, GL_UNSIGNED_BYTE, texImage);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 5, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, texImage);

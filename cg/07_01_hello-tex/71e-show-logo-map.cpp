...
const GLfloat map_size_y = map_size_x * (GLfloat)LOGO_MAP_HEIGHT / (GLfloat)LOGO_MAP_WIDTH;
...
void initTex(void) {
	...
	if ((use_new_tex_syntax == true) && glewIsSupported("GL_VERSION_4_5")) {
		...
		glTextureStorage2D(texObj, 1, GL_RGBA8, LOGO_MAP_WIDTH, LOGO_MAP_HEIGHT);
		glTextureSubImage2D(texObj, 0, 0, 0, LOGO_MAP_WIDTH, LOGO_MAP_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, logo_map);

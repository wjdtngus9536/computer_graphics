...
const char* vertFileName = "71-single-tex.vert";
const char* fragFileName = "71-single-tex.frag";
...
#include "moon-map.cpp"

// image size_y = 1024 x 588 -> y size = (588 / 1024) * size_x
const GLfloat map_size_x = 0.9f;
const GLfloat map_size_y = map_size_x * (GLfloat)MOON_MAP_HEIGHT / (GLfloat)MOON_MAP_WIDTH;
...
void initTex(void) {
	...
	if ((use_new_tex_syntax == true) && glewIsSupported("GL_VERSION_4_5")) {
		printf("using OpenGL 4.5 syntax\n");
		glTextureStorage2D(texObj, 1, GL_RGBA8, MOON_MAP_WIDTH, MOON_MAP_HEIGHT);
		glTextureSubImage2D(texObj, 0, 0, 0, MOON_MAP_WIDTH, MOON_MAP_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, moon_map);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, MOON_MAP_WIDTH, MOON_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, moon_map);

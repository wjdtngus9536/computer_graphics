...
const GLfloat map_size_y = map_size_x * (GLfloat)WORLD_MAP_HEIGHT / (GLfloat)WORLD_MAP_WIDTH;
...
void initTex(void) {
	...
	if ((use_new_tex_syntax == true) && glewIsSupported("GL_VERSION_4_5")) {
		...
		glTextureStorage2D(texObj, 1, GL_RGBA8, WORLD_MAP_WIDTH, WORLD_MAP_HEIGHT);
		glTextureSubImage2D(texObj, 0, 0, 0, WORLD_MAP_WIDTH, WORLD_MAP_HEIGHT, GL_RGBA, GL_UNSIGNED_BYTE, world_map);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, WORLD_MAP_WIDTH, WORLD_MAP_HEIGHT, 0,
		             GL_RGBA, GL_UNSIGNED_BYTE, world_map);

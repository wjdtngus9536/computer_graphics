...
glm::vec2 dragEnd; // mouse dragging end point
glm::vec2 dragFinal; // mouse dragging final vector
...
void cursorPosFunc(GLFWwindow* win, double xscr, double yscr) {
	if (mousePressed == GL_TRUE) {
		glm::vec2 dragCur = glm::vec2((GLfloat)xscr, (GLfloat)yscr) - dragStart;
		printf("dragging vector: %f,%f\n", dragCur.x, dragCur.y);
		fflush(stdout);
	}
}
...
void mouseButtonFunc(GLFWwindow* win, int button, int action, int mods) {
	...
	case GLFW_PRESS:
		...
		printf("dragging start: %f,%f\n", dragStart.x, dragStart.y);
		break;
	case GLFW_RELEASE:
		mousePressed = GL_FALSE;
		glfwGetCursorPos(win, &x, &y);
		dragEnd = glm::vec2((GLfloat)x, (GLfloat)y);
		printf("dragging end: %f,%f\n", dragEnd.x, dragEnd.y);
		dragFinal = dragEnd - dragStart;
		printf("dragging final vector: %f,%f\n", dragFinal.x, dragFinal.y);
		break;
	}
	fflush(stdout);
}
